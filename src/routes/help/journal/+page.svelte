...
<div>
	...
	<HelpSection title="★マーク（処理待ちの目印）">
		<p>
			仕訳行の左端にある★ボタンをクリックすると、仕訳に★マークを付けられます。請求書の添付待ちなど、後で処理が必要な仕訳の目印として活用できます。
		</p>
		<ul class="mt-2 ml-4 list-disc space-y-1">
			<li>★ボタンをクリックするたびにオン/オフが切り替わります</li>
			<li>
				検索ボックス横の★ボタン、または検索クエリに ★
				を入力すると、★マーク付きの仕訳のみを表示できます
			</li>
		</ul>
		<HelpNote type="tip">
			<p>
				「証憑が届いたら添付する」「後で科目を見直す」など、未完了タスクの管理に便利です。
			</p>
		</HelpNote>
	</HelpSection>

	<HelpSection title="お気に入り勘定科目">
		<p>よく使う勘定科目をお気に入りに登録すると、科目選択ダイアログの上部に表示されます。</p>
		<ul class="mt-2 ml-4 list-disc space-y-1">
			<li>科目選択ダイアログ内の各科目の☆ボタンで、その場でお気に入りに追加/解除できます</li>
			<li>勘定科目管理ページ（/accounts）でも☆ボタンで設定できます</li>
			<li>お気に入りがある場合、カテゴリ別の科目一覧は折りたたまれ、▼で展開できます</li>
		</ul>
	</HelpSection>

	<HelpSection title="証憑未添付のハイライト">
		<p>
			経費の仕訳で証跡ステータスが「なし」（○アイコン）のものは、<strong
				>アンバー色（黄色）でハイライト表示</strong
			>されます。領収書の添付漏れに気づきやすくなります。
		</p>
		<ul class="mt-2 ml-4 list-disc space-y-1">
			<li>証憑PDFを添付するか、証跡ステータスを「紙で保管」に変更するとハイライトは消えます</li>
			<li>経費（費用科目）を含まない仕訳（口座間の振替など）はハイライトされません</li>
			<li>
				電車・バスなど領収書が発行されない取引は、勘定科目の編集で「<strong>証憑不要科目</strong
				>」をオンにするとハイライト対象から除外できます（例: 旅費交通費）
			</li>
		</ul>
	</HelpSection>
	...
	<HelpSection title="AIアシスタント（仕訳の相談）">
		...
		<ul class="mt-2 ml-4 list-disc space-y-1">
			...
			<li>
				<strong>過去の仕訳（直近最大100件）を参考にする</strong
				>ため、同様の取引はいつもの科目・パターンで提案されます
			</li>
			<li>家事按分や源泉徴収などの複合仕訳にも対応します</li>
			<li>
				チャット履歴は端末（ブラウザ）に保存され、次回も続きから相談できます（🗑ボタンでクリア可能）
			</li>
		</ul>
	...
	<HelpSection title="CSV・印刷出力">
		<p>仕訳帳画面から直接CSV出力や印刷ができます。画面右上のボタンを使用してください。</p>
		<HelpTable
			headers={['ボタン', '機能']}
			rows={[
		...
		<HelpNote type="info">
			<p>
				検索結果を絞り込んだ状態でも出力できます。
				例：「12月」で検索して12月分だけを印刷することができます。
			</p>
		</HelpNote>
		<HelpNote type="tip">
			<p>複数の帳簿をまとめて出力する場合は、サイドバーの「帳簿出力」ページをご利用ください。</p>
		</HelpNote>
	</HelpSection>
</div>
