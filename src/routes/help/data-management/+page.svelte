...
<div>
	...
	<HelpSection title="データ管理の構成">
		<p>データ管理は以下の機能で構成されています。</p>
		<HelpTable
			headers={['機能', '形式', '対象', '用途']}
			rows={[
	...
	<HelpSection title="CSVインポート">
		<p>ExcelなどのCSVファイルから仕訳を一括登録できます。</p>
		<HelpTable
			headers={['列名', '必須', '説明']}
			rows={[
		...
		/>
		<p class="mt-4">
			複合仕訳は2行目以降の「日付」列を空欄にすることで、同一の仕訳に複数の借方・貸方明細を追加できます。
		</p>
		<HelpNote type="tip">
			<p>
				データ管理ページで「テンプレートCSV」と「勘定科目コード一覧」をダウンロードできます。Excelで編集後「CSV
				UTF-8」形式で保存してからインポートしてください。
			</p>
		</HelpNote>
		<HelpNote type="info">
			<p>
				添付ファイル（証憑PDF）はCSVインポートの対象外です。インポート後に仕訳帳から個別に追加してください。
			</p>
		</HelpNote>
	</HelpSection>
